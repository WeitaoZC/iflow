'''
only for the goal point's tangent space
'''
import os, torch
...
def nearest_pair(X,Y):
    ...
    for i in range(short.shape[0]):
        # d = []
        # for j in range(long.shape[0]):
        #     d.append(np.linalg.norm(short[i]-X[j]))
        # ind = np.argmin(d)
        ind = np.argmin(np.linalg.norm(long-short[i],axis = 1))
        inds.append(ind)
    return inds
